...
namespace CGames
{
    public class WoodenBox : DestroyableObstacle
    {
        [Header("Wooden Box Settings")]
        [SerializeField] private float delayDuration = 0.1f;
        [SerializeField] private float fallingDuration = 0.15f;
        [Space]
        [SerializeField] private float fallHeight = -0.15f;
        ...
        private IEnumerator DropParticlesAfterDelay()
        {
            ...
            StartCoroutine(TransformVFX.MoveObject(this.transform, new(0, fallHeight, 0), fallingDuration));
        }
    }
}