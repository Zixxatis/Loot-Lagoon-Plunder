...
namespace CGames
{
    ...
    {
        [UnityEngine.Serialization.FormerlySerializedAs("_localizationKeyField")]
        ...
        private string GetStringWithValues(string originalText)
        {
            ...
            {
                Debug.LogError($"Couldn't find correct 'start' & 'end' for value field.");
                return originalText;
            }
            ...
            {
                Debug.LogError($"Given String (\"{originalText}\") could not be parsed.");
                return originalText;
            }
            ...
            if(valuesList.Count - 1 < valueIndex || (valuesList[valueIndex] == null))
            {
                if(Application.isPlaying)
                    Debug.LogWarning($"Couldn't get values for {valueIndex} - Func() is null.");
